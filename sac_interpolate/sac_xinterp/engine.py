...
# ---------- write SAC ----------
def write_windowed_like(template_path, out_path, y, window_cfg, S_v, rayp_v, comp):
    st = read(template_path)
    tr = st[0]

    # keep header as template
    new = tr.copy()
    new.data = y.astype(np.float32)

    # compute the new begin time
    B = float(getattr(tr.stats.sac, 'b', 0.0) if hasattr(tr.stats, 'sac') else 0.0)
    fs = 1.0 / tr.stats.delta
    slice_start = S_v + window_cfg.tmin    # absolute start of the slice
    new.stats.sac.b = slice_start
    new.stats.starttime = tr.stats.starttime + (slice_start - B)

    new.stats.sac.user0 = S_v
    new.stats.sac.user1 = rayp_v
    new.stats.kcmpnm = comp.upper()
